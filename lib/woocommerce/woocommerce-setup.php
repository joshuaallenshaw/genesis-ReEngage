 ...
 * Genesis ReEngage.
 *
 * This file adds the required WooCommerce setup functions to the Genesis ReEngage Theme.
 *
...
add_filter( 'woocommerce_style_smallscreen_breakpoint', 'genesis_reengage_woocommerce_breakpoint' );
/**
 ...
 */
function genesis_reengage_woocommerce_breakpoint() {
...
add_filter( 'genesiswooc_products_per_page', 'genesis_reengage_default_products_per_page' );
/**
 ...
 */
function genesis_reengage_default_products_per_page() {
	return 8;

}

add_filter( 'woocommerce_pagination_args', 'genesis_reengage_woocommerce_pagination' );
/**
 ...
 */
function genesis_reengage_woocommerce_pagination( $args ) {
	$args['prev_text'] = sprintf( '&laquo; %s', __( 'Previous Page', 'genesis-reengage' ) );
	$args['next_text'] = sprintf( '%s &raquo;', __( 'Next Page', 'genesis-reengage' ) );

	return $args;

}

add_action( 'after_switch_theme', 'genesis_reengage_woocommerce_image_dimensions_after_theme_setup', 1 );
/**
 * Defines WooCommerce image sizes on theme activation.
 *
 * @since 2.3.0
 */
function genesis_reengage_woocommerce_image_dimensions_after_theme_setup() {
	global $pagenow;
	...
	genesis_reengage_update_woocommerce_image_dimensions();

}

add_action( 'activated_plugin', 'genesis_reengage_woocommerce_image_dimensions_after_woo_activation', 10, 2 );
/**
 ...
 */
function genesis_reengage_woocommerce_image_dimensions_after_woo_activation( $plugin ) {
 ...
 * @since 2.3.0
 */
function genesis_reengage_update_woocommerce_image_dimensions() {
...
add_filter( 'woocommerce_get_image_size_gallery_thumbnail', 'genesis_reengage_gallery_image_thumbnail' );
/**
 ...
 */
function genesis_reengage_gallery_image_thumbnail( $size ) {
	$size = [
		'width'  => 180,
		'height' => 180,
		'crop'   => 1,

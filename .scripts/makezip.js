 ...
 */
const renameMarkdownFilesToTxt = new Promise(
    function (resolve, reject) {
        console.log(chalk`{cyan Renaming .md files to .txt}`);
        ["CHANGELOG.md", "README.md", "CONTRIBUTING.md"].forEach(
            function (file) {
...
const zipFiles = function () {
    recursive(
        process.cwd(), excludes, function (err, files) {
            let relativePath;
...
const renameTxtFilesToMarkdown = function () {
    ...
    ["CHANGELOG.txt", "README.txt", "CONTRIBUTING.txt"].forEach(
        function (file) {
            if (fs.existsSync(file)) {
